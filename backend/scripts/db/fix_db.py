...
def main():
    print("[START] Iniciando proceso de arreglo de base de datos...")

    # Verificar que DATABASE_URL existe
    database_url = os.environ.get('DATABASE_URL')
    if not database_url:
        print("[ERROR] ERROR: DATABASE_URL no está configurado")
        sys.exit(1)

    print(f"[STATS] DATABASE_URL encontrado")

    # Instalar psql si no existe
    print("\n[PKG] Verificando/Instalando postgresql-client...")
    run_command(
    ...
    print("\n[CONFIG] Arreglando permisos de PostgreSQL...")
    ...
    if not success:
        print("\n[WARN]  Los permisos pueden ya estar configurados o necesitas privilegios de admin")
        print("    Intentando continuar con las migraciones de todas formas...")

    # Correr migraciones de Django
    print("\n[PKG] Corriendo migraciones de Django...")
    success = run_command(
    ...
    if success:
        print("\n" + "="*60)
        print("[OK] ¡TODO COMPLETADO CON ÉXITO!")
        print("="*60)
        print("\n[DONE] Ahora puedes:")
        ...
        print("\n" + "="*60)
        print("[ERROR] Las migraciones fallaron")
        print("="*60)
        print("\nPosibles soluciones:")
